...
}else
{
    ...
    if ($foto)
    {
      if (copy($ruta,$destino))
      {
        ...
        if (mysql_affected_rows() > 0)
        {
          ...
          echo '<script>location.href = "../add_vehicle.php?error=true&serie='.$serie.'&tipo='.$tipo.'&modelo='.$modelo.'&marca='.$marca.'&cilindros='.$cilindros.'&color='.$color.'&engomado='.$engomado.'&f_expedicion='.$f_expedicion.'&f_vencimiento='.$f_vencimiento.'&titular='.$titular.'&sucursal='.$suc.'&linea='.$linea.'"</script>';
        }
      }else {
          echo '<script>location.href = "../add_vehicle.php?error_image=true&serie='.$serie.'&tipo='.$tipo.'&modelo='.$modelo.'&marca='.$marca.'&cilindros='.$cilindros.'&color='.$color.'&engomado='.$engomado.'&f_expedicion='.$f_expedicion.'&f_vencimiento='.$f_vencimiento.'&titular='.$titular.'&sucursal='.$suc.'&linea='.$linea.'"</script>';
      }
    }else {
  ...
  {
      echo '<script>location.href = "../add_vehicle.php?engomado_exist=true&serie='.$serie.'&tipo='.$tipo.'&modelo='.$modelo.'&marca='.$marca.'&cilindros='.$cilindros.'&color='.$color.'&engomado='.$engomado.'&f_expedicion='.$f_expedicion.'&f_vencimiento='.$f_vencimiento.'&titular='.$titular.'&sucursal='.$suc.'&linea='.$linea.'"</script>';
  }
}
?>
