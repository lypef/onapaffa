...
<table class="table striped">
    ...
    $sql = "SELECT id FROM titulares WHERE atendio = '$_SESSION[usuario]' ";
    ...
    $sql = "SELECT t.id, t.fotografia, t.nombre, s.nombre, t.domicilio, t.cp, t.telefono FROM titulares  t, sucursales s where t.sucursal = s.id and t.atendio = '$_SESSION[usuario]' ORDER BY id DESC LIMIT ".$inicio.",". "$TAMANO_PAGINA";

    $result = mysqli_query($conn,$sql);

    echo ' Total de altas: ('.$num_total_registros.')';
    while($row = mysqli_fetch_array($result)){
    echo "
    <tr>
        <td>".$row[0]."</td>
        <td>".'<img class= "round100" src="'.$row[1].'">'." $row[2]</td>
        <td>".$row[3]."</td>
        <td>".
      ...
      '
        ...
        <div class='dialog' data-role='dialog' id=".$row[0].">
            <div class='dialog-title'>".'<center><img class= "round100" src="'.$row[1].'">'."
            ".$row[2]."</center></div>
            <div class='dialog-content'>
                DOMICILIO: ".$row[4].", CP: ".$row[5]."
                <br>TELEFONO: ".$row[6]."
                <br>SUCURSAL: ".$row[3]."
            </div>
